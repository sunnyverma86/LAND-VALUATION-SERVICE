...
@Service
public class MasterDataServiceImpl2 extends AbstractMasterRepository implements MasterDataService2 {

	Logger log = LoggerFactory.getLogger(MasterDataServiceImpl2.class);
	...
	public ResponseModel getCircleByDistrict(String districtCode) {
		...
		try {
			...
			// Log repository invocation
			log.debug(
					"PROCESSING | [{}] | Invoking repository: circleDetailsRepo.findCircleByDistrictCodeAndActiveTrue",
					methodName);
	...
	public ResponseModel getMouzaDetails(String districtCode, String circleCode) {
		...
		try {
			if (((districtCode == null || districtCode.trim().equals(""))
				...
				String status = ModelConstant.COMPLETE;
				log.debug(
						"PROCESSING | [{}] | Invoking repository: mouzaDetailsRepo.findMouzaByDistrictCodeAndCircleCodeAndActiveTrue",
						methodName);
	...
	public ResponseModel getLotByDistrictAndCircleAndMouza(String districtCode, String circleCode, String mouzaCode) {
		...
		try {
			...
			List<LotDetails> lotList = lotDetailsRepo
					.findLotByDistrictCodeAndCircleCodeAndMouzaCodeAndActiveTrue(districtCode, circleCode, mouzaCode);
	...
	public ResponseModel getVillageByDistrictAndCircleAndMauzaAndLot(String districtCode, String circleCode,
		...
		try {
			...
			log.debug(
					"[PROCESS] [{}] - Invoking repository: findVillageByDistrictCodeAndCircleCodeAndMauzaAndLotAndActiveTrue | Params: districtCode={}, circleCode={}, mauza={}, lot={}, status={}",
					methodName, districtCode, circleCode, mauza, lot, status);

			List<VillageDetails> villageList = villageDetailsRepo
					.findVillageByDistrictCodeAndCircleCodeAndMouzaCodeAndLotCodeAndActiveTrue(districtCode, circleCode,
							mauza, lot);
	...
	public ResponseModel getAllLandsCategoryDetails() {
		...
		try {
			...
			String Status = ModelConstant.COMPLETE;
			// Log repository invocation
			log.debug("PROCESSING | [{}] | Invoking repository: landsCategoryDetailsRepo.findAllByActiveTrue",
					methodName);
			List<LandsCategoryDetails> landsCategoryList = landsCategoryDetailsRepo.findAllByActiveTrue();
	...
	public ResponseModel getParameterDetailsAll() {
		...
		try {
			// Log repository invocation
			log.debug("PROCESSING | [{}] | Invoking repository: parameterDetailsRepo.findAllByActiveTrue()",
					methodName);

			String status = ModelConstant.COMPLETE;
			List<ParameterDetails> parameterList = parameterDetailsRepo.findAllByActiveTrue();
	...
	@Override
	public ResponseModel getSubParameterDetailsAllByParameterCodeAndDistrictCode(String parameterCode,
			String districtCode) {

		final String methodName = "getSubParameterDetailsAllByParameterCodeAndDistrictCode";
		ResponseModel response = new ResponseModel();
		...
		try {
			...
			log.debug(
					"PROCESSING | {} | Calling repository: "
							+ "findAllByActiveTrueAndStatusAndParameterCodeAndDistrictCode "
							+ "| Status: {} | Parameter Code: {} | District Code: {}",
					methodName, ModelConstant.COMPLETE, parameterCode, districtCode);

			List<SubParameterDetails> subParameterList = subParameterDetailsRepo
					.findAllByActiveTrueAndStatusAndParameterCodeAndDistrictCode(ModelConstant.COMPLETE, parameterCode,
							districtCode);

			if (subParameterList == null || subParameterList.isEmpty()) {

				log.warn("NO DATA | {} | No SubParameter Details found | Parameter Code: {} | District Code: {}",
						methodName, parameterCode, districtCode);

				response.setHttpStatus(HttpStatus.NO_CONTENT);
				response.setMessage("No SubParameter Details found");
				return response;
			}

			response.setData(subParameterList);
			response.setHttpStatus(HttpStatus.OK);
			response.setMessage("SubParameter Details fetched successfully");

			log.info(
					"SUCCESS | {} | SubParameter Details fetched | Parameter Code: {} | District Code: {} | Records: {}",
					methodName, parameterCode, districtCode, subParameterList.size());
		...
		log.info("END | {} | Parameter Code: {} | District Code: {} | Response Status: {} | Message: {}", methodName,
				parameterCode, districtCode, response.getHttpStatus(), response.getMessage());
	...
	public ResponseModel addDistrictDetails(DistrictDetailsModelRequest districtDetailsModel) {
		...
		try {
			...
			districtDetails.setRequestStatus(ModelConstant.ADD);
			...
			if (userRoles.contains(ModelConstant.ADMIN)) {
				...
				districtDetails.setStatus(ModelConstant.PEN_DC);
				districtDetails.setStatusCode(ModelConstant.PEN_DC_CODE);
			} else if (userRoles.contains(ModelConstant.CO)) {
				districtDetails.setActive(false);
				districtDetails.setStatus(ModelConstant.PEN_ADC);
				districtDetails.setStatusCode(ModelConstant.PEN_ADC_CODE);
			} else if (userRoles.contains(ModelConstant.LRA)) {
				districtDetails.setActive(false);
				districtDetails.setStatus(ModelConstant.PEN_CO);
				districtDetails.setStatusCode(ModelConstant.PEN_CO_CODE);
			} else {
				districtDetails.setActive(false);
				districtDetails.setStatus(ModelConstant.PEN_LRA);
				districtDetails.setStatusCode(ModelConstant.PEN_LRA_CODE);
			}
	...
	public ResponseModel updateDistrictDetails(DistrictDetailsModelRequest districtDetailsModel) {
		...
		try {
			...
			if (userRoles.contains(ModelConstant.ADMIN)) {
				...
				existingDistrict.setActive(true);
			} else if (userRoles.contains(ModelConstant.CO)) {
				existingDistrict.setActive(true);
			} else if (userRoles.contains(ModelConstant.LRA)) {
				existingDistrict.setActive(true);
			...
			districtDetails.setRequestStatus(ModelConstant.UPDATE);
	...
	public ResponseModel deleteDistrictDetails(String districtCode) {
		...
		try {
			...
			// Mark as inactive
			// districtDetails.setActive(false);
			districtDetails.setRequestStatus(ModelConstant.DELETE);
			districtDetails.setUpdatedBy(loginId);
			districtDetails.setUpdatedDtm(new Timestamp(System.currentTimeMillis()));
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
	...
	public ResponseModel addCircleDetails(CircleDetailsModelRequest circleDetailsModel) {
		...
		try {
			...
			circleDetails.setRequestStatus(ModelConstant.ADD);
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
				circleDetails.setActive(true);
				...
				circleDetails.setStatus(ModelConstant.PEN_DC);
				circleDetails.setStatusCode(ModelConstant.PEN_DC_CODE);
			} else if (userRoles.contains(ModelConstant.CO)) {
				circleDetails.setActive(false);
				circleDetails.setStatus(ModelConstant.PEN_ADC);
				circleDetails.setStatusCode(ModelConstant.PEN_ADC_CODE);
			} else if (userRoles.contains(ModelConstant.LRA)) {
				circleDetails.setActive(false);
				circleDetails.setStatus(ModelConstant.PEN_CO);
				circleDetails.setStatusCode(ModelConstant.PEN_CO_CODE);
			} else {
				circleDetails.setActive(false);
				circleDetails.setStatus(ModelConstant.PEN_LRA);
				circleDetails.setStatusCode(ModelConstant.PEN_LRA_CODE);
			}
	...
	public ResponseModel updateCircleDetails(CircleDetailsModelUpdateRequest circleDetailsModel) {
		...
		try {
			...
			circleDetails.setDistrictCode(circleDetailsModel.getDistrictCode());
			// circleDetails.setActive(true);
			circleDetails.setRequestStatus(ModelConstant.UPDATE);
	...
	public ResponseModel deleteCircleDetails(String circleCode) {
		...
		try {
			...
			// Mark as inactive
			// circleDetails.setActive(false)
			circleDetails.setRequestStatus(ModelConstant.DELETE);
			circleDetails.setUpdatedBy(loginId);
			circleDetails.setUpdatedDtm(new Timestamp(System.currentTimeMillis()));
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
	...
	public ResponseModel addMouzaDetails(MouzaDetailsModelRequest mouzaDetailsModel) {
		...
		try {
			...
			mouzaDetails.setRequestStatus(ModelConstant.ADD);
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
				mouzaDetails.setActive(true);
				...
				mouzaDetails.setStatus(ModelConstant.PEN_DC);
				mouzaDetails.setStatusCode(ModelConstant.PEN_DC_CODE);
			} else if (userRoles.contains(ModelConstant.CO)) {
				mouzaDetails.setActive(false);
				mouzaDetails.setStatus(ModelConstant.PEN_ADC);
				mouzaDetails.setStatusCode(ModelConstant.PEN_ADC_CODE);
			} else if (userRoles.contains(ModelConstant.LRA)) {
				mouzaDetails.setActive(false);
				mouzaDetails.setStatus(ModelConstant.PEN_CO);
				mouzaDetails.setStatusCode(ModelConstant.PEN_CO_CODE);
			} else {
				mouzaDetails.setActive(false);
				mouzaDetails.setStatus(ModelConstant.PEN_LRA);
				mouzaDetails.setStatusCode(ModelConstant.PEN_LRA_CODE);
			}
	...
	public ResponseModel updateMouzaDetails(MouzaDetailsModelUpdateRequest mouzaDetailsModel) {
		...
		try {
			...
			mouzaDetails.setRequestStatus(ModelConstant.UPDATE);
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
				mouzaDetails.setActive(true);
				mouzaDetails.setStatus(ModelConstant.COMPLETE);
	...
	public ResponseModel deleteMouzaDetails(String mouzaCode) {
		...
		try {
			...
			mouzaDetails.setUpdatedBy(loginId);
			mouzaDetails.setUpdatedDtm(new Timestamp(System.currentTimeMillis()));
			mouzaDetails.setRequestStatus(ModelConstant.DELETE);
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
				mouzaDetails.setActive(true);
				mouzaDetails.setStatus(ModelConstant.COMPLETE);
	...
	public ResponseModel addLotDetails(LotDetailsModelRequest lotDetailsModel) {
		...
		try {
			...
			lotDetails.setRequestStatus(ModelConstant.ADD);
			...
			if (userRoles.contains(ModelConstant.ADMIN)) {
				...
				lotDetails.setStatus(ModelConstant.PEN_DC);
				lotDetails.setStatusCode(ModelConstant.PEN_DC_CODE);
			} else if (userRoles.contains(ModelConstant.CO)) {
				lotDetails.setActive(false);
				lotDetails.setStatus(ModelConstant.PEN_ADC);
				lotDetails.setStatusCode(ModelConstant.PEN_ADC_CODE);
			} else if (userRoles.contains(ModelConstant.LRA)) {
				lotDetails.setActive(false);
				lotDetails.setStatus(ModelConstant.PEN_CO);
				lotDetails.setStatusCode(ModelConstant.PEN_CO_CODE);
			} else {
				lotDetails.setActive(false);
				lotDetails.setStatus(ModelConstant.PEN_LRA);
				lotDetails.setStatusCode(ModelConstant.PEN_LRA_CODE);
			}
	...
	public ResponseModel updateLotDetails(LotDetailsModelUpdateRequest lotDetailsModel) {
		...
		try {
			...
			lotDetails.setRequestStatus(ModelConstant.UPDATE);
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
				lotDetails.setActive(true);
				lotDetails.setStatus(ModelConstant.COMPLETE);
	...
	public ResponseModel deleteLotDetails(String lotCode) {
		...
		try {
			...
			lotDetails.setUpdatedBy(loginId);
			lotDetails.setUpdatedDtm(new Timestamp(System.currentTimeMillis()));
			lotDetails.setRequestStatus(ModelConstant.DELETE);
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
				lotDetails.setActive(true);
				lotDetails.setStatus(ModelConstant.COMPLETE);
	...
	public ResponseModel addVillageDetails(VillageDetailsModelRequest villageDetailsModel) {
		...
		try {
			...
			villageDetails.setRequestStatus(ModelConstant.ADD);
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
				villageDetails.setActive(true);
				...
				villageDetails.setStatus(ModelConstant.PEN_DC);
				villageDetails.setStatusCode(ModelConstant.PEN_DC_CODE);
			} else if (userRoles.contains(ModelConstant.CO)) {
				villageDetails.setActive(false);
				villageDetails.setStatus(ModelConstant.PEN_ADC);
				villageDetails.setStatusCode(ModelConstant.PEN_ADC_CODE);
			} else if (userRoles.contains(ModelConstant.LRA)) {
				villageDetails.setActive(false);
				villageDetails.setStatus(ModelConstant.PEN_CO);
				villageDetails.setStatusCode(ModelConstant.PEN_CO_CODE);
			} else {
				villageDetails.setActive(false);
				villageDetails.setStatus(ModelConstant.PEN_LRA);
				villageDetails.setStatusCode(ModelConstant.PEN_LRA_CODE);
			}
			// Save record
	...
	public ResponseModel updateVillageDetails(VillageDetailsModelUpdateRequest villageDetailsModel) {
		...
		try {
			...
			villageDetails.setRequestStatus(ModelConstant.UPDATE);
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
				villageDetails.setActive(true);
				villageDetails.setStatus(ModelConstant.COMPLETE);
	...
	public ResponseModel addLandsCategory(String landCategoryName) {
		...
		try {
			...
			landsCategoryDetails.setRequestStatus(ModelConstant.ADD);
			...
			if (userRoles.contains(ModelConstant.ADMIN)) {
				...
				landsCategoryDetails.setActive(false);
				landsCategoryDetails.setStatus(ModelConstant.PEN_DC);
				landsCategoryDetails.setStatusCode(ModelConstant.PEN_DC_CODE);
			} else if (userRoles.contains(ModelConstant.CO)) {
				landsCategoryDetails.setActive(false);
				landsCategoryDetails.setStatus(ModelConstant.PEN_ADC);
				landsCategoryDetails.setStatusCode(ModelConstant.PEN_ADC_CODE);
			} else if (userRoles.contains(ModelConstant.LRA)) {
				landsCategoryDetails.setActive(false);
				landsCategoryDetails.setStatus(ModelConstant.PEN_CO);
				landsCategoryDetails.setStatusCode(ModelConstant.PEN_CO_CODE);
			} else {
				landsCategoryDetails.setActive(false);
				landsCategoryDetails.setStatus(ModelConstant.PEN_LRA);
				landsCategoryDetails.setStatusCode(ModelConstant.PEN_LRA_CODE);
			}
	...
	public ResponseModel deleteLandsCategory(String landCategoryCode) {
		...
		try {
			...
			landCategories.setRequestStatus(ModelConstant.DELETE);
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
				...
				landCategories.setActive(false);
				landCategories.setStatus(ModelConstant.PEN_DC);
				landCategories.setStatusCode(ModelConstant.PEN_DC_CODE);
			} else if (userRoles.contains(ModelConstant.CO)) {
				landCategories.setActive(false);
				landCategories.setStatus(ModelConstant.PEN_ADC);
				landCategories.setStatusCode(ModelConstant.PEN_ADC_CODE);
			} else if (userRoles.contains(ModelConstant.LRA)) {
				landCategories.setActive(false);
				landCategories.setStatus(ModelConstant.PEN_CO);
				landCategories.setStatusCode(ModelConstant.PEN_CO_CODE);
			} else {
				landCategories.setActive(false);
				landCategories.setStatus(ModelConstant.PEN_LRA);
				landCategories.setStatusCode(ModelConstant.PEN_LRA_CODE);
			}
	...
	public ResponseModel addLandSubClassDetails(LandSubClassDetailsModelRequest landSubClassDetailsModel) {
		...
		try {
			...
			landSubClassDetails.setRequestStatus(ModelConstant.ADD);
			...
			if (userRoles.contains(ModelConstant.ADMIN)) {
				...
				landSubClassDetails.setStatus(ModelConstant.PEN_DC);
				landSubClassDetails.setStatusCode(ModelConstant.PEN_DC_CODE);
			} else if (userRoles.contains(ModelConstant.CO)) {
				landSubClassDetails.setActive(false);
				landSubClassDetails.setStatus(ModelConstant.PEN_ADC);
				landSubClassDetails.setStatusCode(ModelConstant.PEN_ADC_CODE);
			} else if (userRoles.contains(ModelConstant.LRA)) {
				landSubClassDetails.setActive(false);
				landSubClassDetails.setStatus(ModelConstant.PEN_CO);
				landSubClassDetails.setStatusCode(ModelConstant.PEN_CO_CODE);
			} else {
				landSubClassDetails.setActive(false);
				landSubClassDetails.setStatus(ModelConstant.PEN_LRA);
				landSubClassDetails.setStatusCode(ModelConstant.PEN_LRA_CODE);
			}
	...
	public ResponseModel updateLandSubClassDetails(LandSubClassDetailsModelUpdateRequest landSubClassDetailsModel) {
		...
		try {
			...
			if (userRoles.contains(ModelConstant.ADMIN)) {
				...
				existingLandSubClassDetails.setActive(true);
			} else if (userRoles.contains(ModelConstant.CO)) {
				existingLandSubClassDetails.setActive(true);
			} else if (userRoles.contains(ModelConstant.LRA)) {
				existingLandSubClassDetails.setActive(true);
			...
			landSubClassDetails.setRequestStatus(ModelConstant.UPDATE);
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
				landSubClassDetails.setActive(true);
				landSubClassDetails.setStatus(ModelConstant.COMPLETE);
	...
	public ResponseModel deleteLandSubClassDetails(String landSubClass) {
		...
		try {
			...
			landSubClassDetails.setUpdatedBy(loginId);
			landSubClassDetails.setUpdatedDtm(new Timestamp(System.currentTimeMillis()));
			landSubClassDetails.setRequestStatus(ModelConstant.DELETE);
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
				landSubClassDetails.setActive(true);
				landSubClassDetails.setStatus(ModelConstant.COMPLETE);
	...
	public ResponseModel addParameterDetails(ParameterDetailsModelRequest parameterDetailsModel) {
		...
		try {
			...
			if (userRoles.contains(ModelConstant.ADMIN)) {
				...
				parameterDetails.setStatus(ModelConstant.PEN_DC);
				parameterDetails.setStatusCode(ModelConstant.PEN_DC_CODE);
			} else if (userRoles.contains(ModelConstant.CO)) {
				parameterDetails.setActive(false);
				parameterDetails.setStatus(ModelConstant.PEN_ADC);
				parameterDetails.setStatusCode(ModelConstant.PEN_ADC_CODE);
			} else if (userRoles.contains(ModelConstant.LRA)) {
				parameterDetails.setActive(false);
				parameterDetails.setStatus(ModelConstant.PEN_CO);
				parameterDetails.setStatusCode(ModelConstant.PEN_CO_CODE);
			} else {
				parameterDetails.setActive(false);
				parameterDetails.setStatus(ModelConstant.PEN_LRA);
				parameterDetails.setStatusCode(ModelConstant.PEN_LRA_CODE);
			}
	...
	public ResponseModel updateParameterDetails(ParameterDetailsModelUpdateRequest parameterDetailsModel) {
		...
		try {
			...
			if (userRoles.contains(ModelConstant.ADMIN)) {
				...
				existingParameter.setActive(true);
			} else if (userRoles.contains(ModelConstant.CO)) {
				existingParameter.setActive(true);
			} else if (userRoles.contains(ModelConstant.LRA)) {
				existingParameter.setActive(true);
			...
			parameterDetails.setRequestStatus(ModelConstant.UPDATE);
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
				parameterDetails.setActive(true);
				parameterDetails.setStatus(ModelConstant.COMPLETE);
	...
	public ResponseModel deleteParameterDetails(String parameterCode) {
		...
		try {
			...
			parameterDetails.setUpdatedBy(loginId);
			parameterDetails.setUpdatedDtm(new Timestamp(System.currentTimeMillis()));
			parameterDetails.setRequestStatus(ModelConstant.DELETE);
			// Determine status based on roles
			if (userRoles.contains(ModelConstant.ADMIN)) {
				parameterDetails.setActive(true);
				parameterDetails.setStatus(ModelConstant.COMPLETE);
...
//			subParameterDetails.setRequestStatus(ModelConstant.ADD);
//			// Determine status based on roles
//			if (userRoles.contains(ModelConstant.ADMIN)) {
//				subParameterDetails.setActive(true);
...
//				subParameterDetails.setStatus(ModelConstant.PEN_DC);
//				subParameterDetails.setStatusCode(ModelConstant.PEN_DC_CODE);
//			} else if (userRoles.contains(ModelConstant.CO)) {
//				subParameterDetails.setActive(false);
//				subParameterDetails.setStatus(ModelConstant.PEN_ADC);
//				subParameterDetails.setStatusCode(ModelConstant.PEN_ADC_CODE);
//			} else if (userRoles.contains(ModelConstant.LRA)) {
//				subParameterDetails.setActive(false);
//				subParameterDetails.setStatus(ModelConstant.PEN_CO);
//				subParameterDetails.setStatusCode(ModelConstant.PEN_CO_CODE);
//			} else {
//				subParameterDetails.setActive(false);
//				subParameterDetails.setStatus(ModelConstant.PEN_LRA);
//				subParameterDetails.setStatusCode(ModelConstant.PEN_LRA_CODE);
//			}
...
//	}
	...
	public ResponseModel addSubParameterDetails(SubParameterDetailsModelRequest subParameterDetailsModel) {
		...
		try {
			log.info("Request received to add SubParameter | "
					+ "SubParameter Name: {} | Parameter Code: {} | Band: {} | "
					+ "District Code: {} | Area Type (R/U): {} | " + "Weightage: {} | Effective From: {} | Method: {}",
					...
					subParameterDetailsModel.getEffectiveFrom(), methodName);
			...
			subParameterDetails.setSubParameterName(subParameterDetailsModel.getSubParameterName());
			subParameterDetails.setParameterCode(subParameterDetailsModel.getParameterCode());
			subParameterDetails.setDistrictCode(subParameterDetailsModel.getDistrictCode());
			subParameterDetails.setAreaTypeRandU(subParameterDetailsModel.getAreaTypeRandU());
			subParameterDetails.setBand(subParameterDetailsModel.getBand());
			subParameterDetails.setBasePriceIncreaseSubParameter(subParameterDetailsModel.getWeightage());
			subParameterDetails.setEffectiveFrom(subParameterDetailsModel.getEffectiveFrom());
			subParameterDetails.setCreatedBy(loginId);
			subParameterDetails.setRequestStatus(ModelConstant.ADD);
			...
			logAction(loginId, ModelConstant.SUBPARAMETER, ModelConstant.ADD,
					"SubParameter Code: " + savedSubParameterDetails.getSubParameterCode(),
					"SubParameter added | Name: " + savedSubParameterDetails.getSubParameterName()
							+ ", Parameter Code: " + savedSubParameterDetails.getParameterCode() + ", Band: "
							+ savedSubParameterDetails.getBand() + ", District Code: "
							+ savedSubParameterDetails.getDistrictCode() + ", Area Type: "
							+ savedSubParameterDetails.getAreaTypeRandU() + ", Weightage: "
							+ savedSubParameterDetails.getBasePriceIncreaseSubParameter() + ", Effective From: "
							+ savedSubParameterDetails.getEffectiveFrom(),
					savedSubParameterDetails.getStatus(), savedSubParameterDetails.getStatusCode(),
					savedSubParameterDetails.getSubParameterGenId());
			...
			log.info("SubParameter added successfully | SubParameter Code: {} | Band: {} | Status: {} | Method: {}",
					savedSubParameterDetails.getSubParameterCode(), savedSubParameterDetails.getBand(),
					savedSubParameterDetails.getStatus(), methodName);

		} catch (Exception e) {

			log.error(
					"Error while adding SubParameter | SubParameter Name: {} | Parameter Code: {} | Band: {} | Method: {}",
					subParameterDetailsModel.getSubParameterName(), subParameterDetailsModel.getParameterCode(),
					subParameterDetailsModel.getBand(), methodName, e);

			response.setHttpStatus(HttpStatus.EXPECTATION_FAILED);
			response.setMessage("Failed to add SubParameter Details | SubParameter Name: "
					+ subParameterDetailsModel.getSubParameterName());
		}
...
//				existingSubParameter.setActive(false);
//			} else if (userRoles.contains(ModelConstant.ADC)) {
//				existingSubParameter.setActive(true);
//			} else if (userRoles.contains(ModelConstant.CO)) {
//				existingSubParameter.setActive(true);
//			}  else if (userRoles.contains(ModelConstant.LRA)) {
//				existingSubParameter.setActive(true);
...
//			subParameterDetails.setRequestStatus(ModelConstant.UPDATE);
//			// Determine status based on roles
//			if (userRoles.contains(ModelConstant.ADMIN)) {
//				subParameterDetails.setActive(true);
//				subParameterDetails.setStatus(ModelConstant.COMPLETE);
...
//	}
	...
	public ResponseModel updateSubParameterDetails(SubParameterDetailsModelUpdateRequest subParameterDetailsModel) {
		...
		try {
			log.info("Request received to update SubParameter | "
					+ "SubParameter Name: {} | SubParameter Code: {} | Parameter Code: {} | "
					+ "District Code: {} | Area Type (R/U): {} | Band: {} | Weightage: {} | Effective From: {} | "
					+ "Method: {} | Class: {}", subParameterDetailsModel.getSubParameterName(),
					subParameterDetailsModel.getSubParameterCode(), subParameterDetailsModel.getParameterCode(),
					subParameterDetailsModel.getDistrictCode(), subParameterDetailsModel.getAreaTypeRandU(),
					subParameterDetailsModel.getBand(), subParameterDetailsModel.getWeightage(),
			...
			if (existingSubParameter == null) {
				log.info("No active SubParameter found | SubParameter Code: {} | Method: {} | Class: {}",
						subParameterDetailsModel.getSubParameterCode(), methodName, this.getClass().getSimpleName());
			...
			existingSubParameter.setUpdatedBy(loginId);
			existingSubParameter.setUpdatedDtm(new Timestamp(System.currentTimeMillis()));
			existingSubParameter.setActive(
					userRoles.contains(ModelConstant.ADMIN) || userRoles.contains(ModelConstant.DC) ? false : true);
			subParameterDetailsRepo.save(existingSubParameter);
			...
			subParameterDetails.setDistrictCode(subParameterDetailsModel.getDistrictCode());
			subParameterDetails.setAreaTypeRandU(subParameterDetailsModel.getAreaTypeRandU());
			subParameterDetails.setBand(subParameterDetailsModel.getBand()); // ✅ NEW
			subParameterDetails.setBasePriceIncreaseSubParameter(subParameterDetailsModel.getWeightage());
			subParameterDetails.setEffectiveFrom(subParameterDetailsModel.getEffectiveFrom());
			...
			logAction(loginId, ModelConstant.SUBPARAMETER, ModelConstant.UPDATE,
					...
					"SubParameter updated | Name: " + savedSubParameter.getSubParameterName() + ", Parameter Code: "
							+ savedSubParameter.getParameterCode() + ", District Code: "
							+ savedSubParameter.getDistrictCode() + ", Area Type: "
							+ savedSubParameter.getAreaTypeRandU() + ", Band: " + savedSubParameter.getBand()
							+ ", Weightage: " + savedSubParameter.getBasePriceIncreaseSubParameter()
							+ ", Effective From: " + savedSubParameter.getEffectiveFrom(),
					savedSubParameter.getStatus(), savedSubParameter.getStatusCode(),
					savedSubParameter.getSubParameterGenId());
			...
			response.setMessage(
					"SubParameter Updated Successfully | SubParameter Name: " + savedSubParameter.getSubParameterName()
							+ ", SubParameter Code: " + savedSubParameter.getSubParameterCode() + ", Parameter Code: "
							+ savedSubParameter.getParameterCode() + ", Band: " + savedSubParameter.getBand());

			log.info(
					"SubParameter updated successfully | SubParameter Code: {} | Band: {} | Updated By: {} | Status: {} | Method: {} | Class: {}",
					savedSubParameter.getSubParameterCode(), savedSubParameter.getBand(), loginId,
					savedSubParameter.getStatus(), methodName, this.getClass().getSimpleName());

		} catch (Exception e) {

			log.error(
					"Error while updating SubParameter | SubParameter Name: {} | SubParameter Code: {} | Parameter Code: {} | Band: {} | Method: {} | Class: {}",
					subParameterDetailsModel.getSubParameterName(), subParameterDetailsModel.getSubParameterCode(),
			...
			response.setMessage("Failed to update SubParameter Details | SubParameter Name: "
					+ subParameterDetailsModel.getSubParameterName() + ", SubParameter Code: "
					+ subParameterDetailsModel.getSubParameterCode());
		}
	...
	public ResponseModel deleteSubParameterDetails(String subParameterCode) {
		...
		try {
			...
			// Mark as inactive
			// subParameterDetails.setActive(false);
			subParameterDetails.setRequestStatus(ModelConstant.DELETE);
			subParameterDetails.setUpdatedBy(loginId);
			subParameterDetails.setUpdatedDtm(new Timestamp(System.currentTimeMillis()));
			subParameterDetails.setRequestStatus(ModelConstant.DELETE);
			// Determine status based on roles
	...
	public ResponseModel addAreaType(AreaTypesModelRequest areaTypesModel) {
		...
		try {
			...
			if (userRoles.contains(ModelConstant.ADMIN)) {
				...
				areaTypes.setStatus(ModelConstant.PEN_DC);
				areaTypes.setStatusCode(ModelConstant.PEN_DC_CODE);
			} else if (userRoles.contains(ModelConstant.CO)) {
				areaTypes.setActive(false);
				areaTypes.setStatus(ModelConstant.PEN_ADC);
				areaTypes.setStatusCode(ModelConstant.PEN_ADC_CODE);
			} else if (userRoles.contains(ModelConstant.LRA)) {
				areaTypes.setActive(false);
				areaTypes.setStatus(ModelConstant.PEN_CO);
				areaTypes.setStatusCode(ModelConstant.PEN_CO_CODE);
			} else {
				areaTypes.setActive(false);
				areaTypes.setStatus(ModelConstant.PEN_LRA);
				areaTypes.setStatusCode(ModelConstant.PEN_LRA_CODE);
			}
